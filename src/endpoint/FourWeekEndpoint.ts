...
export class FourWeekEndpoint implements Endpoint {
  ...
  public* chunk(
  ...
  ): Generator<{ from: DateTime<true>, to: DateTime<true>, complete: boolean, fileName: string, url: string }> {
    ...
    for (let i = firstIndex; i <= lastIndex; i++) {
      ...
      const fileName = `${this.fileName}_${from.toISODate()}_${to.toISODate()}.json`;
      const url = this.urlBuilder(from, to);
      yield { from, to, complete, fileName, url };
    }
  }
}

...
import { FourWeekEndpoint } from './FourWeekEndpoint.js';

// Helpers to build dates relative to the anchor for readable test cases
const anchor = (days: number) => CHUNK_ANCHOR.plus({ days });

// Minimal endpoint instance — URL and schema are irrelevant for chunking tests
const endpoint = new FourWeekEndpoint(() => '', z.unknown(), 'test');

describe('FourWeekEndpoint.chunk', () => {
  describe('chunk boundaries are stable (anchor-based)', () => {
    it('produces the same middle-chunk boundaries regardless of start date', () => {
      ...
      const chunksA = [...endpoint.chunk(anchor(0), anchor(83))]; // start at chunk 0
      const chunksB = [...endpoint.chunk(anchor(10), anchor(83))]; // start mid-chunk 0
  ...
  describe('clamping', () => {
    it('clamps the first chunk from to start when start is mid-chunk', () => {
      ...
      const end = anchor(30);
      const chunks = [...endpoint.chunk(start, end)];
  ...
  describe('coverage and continuity', () => {
    it('yields a single chunk when start and end are in the same chunk', () => {
      const chunks = [...endpoint.chunk(anchor(5), anchor(20))];
      expect(chunks).toHaveLength(1);
    });

    it('consecutive chunks are contiguous (no gaps, no overlaps)', () => {
      const chunks = [...endpoint.chunk(anchor(0), anchor(111))]; // 4 full chunks
    ...
    it('covers exactly 4 chunks when spanning 4 full 28-day windows', () => {
      const chunks = [...endpoint.chunk(anchor(0), anchor(111))]; // 0-27, 28-55, 56-83, 84-111
  ...
  describe('complete flag', () => {
    ...
    it('marks a chunk ending yesterday as complete', () => {
      ...
      const chunks = [...endpoint.chunk(start, yesterday)];
